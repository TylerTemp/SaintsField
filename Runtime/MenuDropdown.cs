...
{
    public class MenuDropdown<T> : IMenuDropdown
    {
        // name, object, disabled, separator
        private readonly List<ValueTuple<string, object, bool, bool>> _values;

        public MenuDropdown() => _values = new List<ValueTuple<string, object, bool, bool>>();
        public MenuDropdown(IEnumerable<ValueTuple<string, T>> value) => _values = value.Select(each => (each.Item1, (object)each.Item2, false, false)).ToList();
        public MenuDropdown(IEnumerable<ValueTuple<string, T, bool>> value) => _values = value.Select(each => (each.Item1, (object)each.Item2, each.Item3, false)).ToList();
        ...
        // public static explicit operator MenuDropdown<object>(MenuDropdown<T> target)
        // {
