...
namespace SaintsField.Editor.Drawers.ProgressBarDrawer
{
    ...
    public partial class ProgressBarAttributeDrawer: SaintsPropertyDrawer
    {
        ...
        private static MetaInfo GetMetaInfo(SerializedProperty property, ISaintsAttribute saintsAttribute, FieldInfo info, object parent)
        {
            ...
            {
                (string error, MemberInfo _, float value) = Util.GetOf(progressBarAttribute.MinCallback, 0f, property, info, parent, null);
                if (error != "")
                {
                    return new MetaInfo
                    {
            ...
            {
                (string error, MemberInfo _, float value) = Util.GetOf(progressBarAttribute.MaxCallback, 0f, property, info, parent, null);
                if (error != "")
                {
                    return new MetaInfo
                    {
        ...
        {
            (string error, MemberInfo _, object value) = Util.GetOf<object>(by, defaultValue, property, fieldInfo, target, null);
            return error != ""
                ? (error, defaultValue)
                : ObjToColor(value);
        }
