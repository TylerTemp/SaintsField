...
namespace SaintsField.Editor.Drawers.ValueButtonsDrawer
{
    public class ValueButtonsRow: VisualElement
    {
        ...
        public void ResetWithButtons(IEnumerable<ValueButtonRawInfo> buttons)
        {
            ...
            foreach (ValueButton toRemove in Util.ShrinkListTo(_buttons, processIndex))
            {
                toRemove.RemoveFromHierarchy();
            }

            foreach ((ValueButton button, int index)  in _buttons.WithIndex())
            {
                if (index == 0)
                {
