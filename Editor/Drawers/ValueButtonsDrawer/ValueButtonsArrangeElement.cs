...
{
    public class ValueButtonsArrangeElement: VisualElement
    {
        private readonly ValueButtonsCalcElement _valueButtonsCalcElement;
        private readonly ValueButtonsRow _mainRow;
        private readonly List<ValueButtonsRow> _subRows = new List<ValueButtonsRow>();
        public readonly UnityEvent<object> OnButtonClicked = new UnityEvent<object>();

        public ValueButtonsArrangeElement()
        {
            style.position = Position.Relative;

            Add(_valueButtonsCalcElement = new ValueButtonsCalcElement
            {
                style =
                {
            ...
            Add(_mainRow = new ValueButtonsRow
            {
                style =
                {
                    // marginTop = -22,
        ...
        private void OnCalcReadyEvent(IReadOnlyList<(IReadOnlyList<RichTextDrawer.RichTextChunk>, float)> results)
        {
            ...
            for (int index = 0; index < splitRowInfos.Count; index++)
            {
                ...
                else
                {
                    ...
                    {
                        var newRow = new ValueButtonsRow();
            ...
            foreach (ValueButtonsRow removed in Util.ShrinkListTo(_subRows, processedIndex))
            {
        ...
        {
            foreach (ValueButtonsRow valueButtonsRow in _subRows.Append(_mainRow))
            {
