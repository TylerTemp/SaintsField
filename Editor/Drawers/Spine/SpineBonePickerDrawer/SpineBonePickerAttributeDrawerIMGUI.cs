...
namespace SaintsField.Editor.Drawers.Spine.SpineBonePickerDrawer
{
    public partial class SpineBonePickerAttributeDrawer
    {
        ...
        private static InfoIMGUI EnsureKey(SerializedProperty property, SpineBonePickerAttribute attribute, FieldInfo info, object parent)
        {
        ...
        private static void RefreshCache(InfoIMGUI cache, SerializedProperty property, SpineBonePickerAttribute attribute, FieldInfo info, object parent)
        {
        ...
        private static AdvancedDropdownMetaInfo GetMetaInfo(string currentValue, SkeletonData skeletonData)
        {
            ...
            {
                BoneData bone = skeletonData.Bones.Items[i];
                options.Add(GetBonePath(bone), bone.Name, icon: IconPath);
            }
        ...
        private static string GetBonePath(BoneData boneData)
        {
            List<string> names = new List<string>();
            for (BoneData iterator = boneData; iterator != null; iterator = iterator.Parent)
            {
        ...
        private static string GetDisplay(string currentValue, SkeletonData skeletonData)
        {
            ...
            for (int i = 0; i < skeletonData.Bones.Count; i++)
            {
                if (skeletonData.Bones.Items[i].Name == currentValue)
                {
        ...
        protected override bool WillDrawBelow(SerializedProperty property, IReadOnlyList<PropertyAttribute> allAttributes,
            ...
            EnsureKey(property, (SpineBonePickerAttribute)saintsAttribute, info, parent).Error != "";
        ...
        {
            string error = EnsureKey(property, (SpineBonePickerAttribute)saintsAttribute, info, parent).Error;
