...
{
    public partial class SpinePathConstraintPickerAttributeDrawer
    {
        ...
        private static InfoIMGUI EnsureKey(SerializedProperty property, SpinePathConstraintPickerAttribute attribute, FieldInfo info, object parent)
        {
        ...
        private static void RefreshCache(InfoIMGUI cache, SerializedProperty property, SpinePathConstraintPickerAttribute attribute, FieldInfo info, object parent)
        {
        ...
        private static AdvancedDropdownMetaInfo GetMetaInfo(string currentValue, SkeletonData skeletonData)
        {
            ...
            foreach (PathConstraintData constraintData in GetPathConstraintData(skeletonData))
            {
                options.Add(constraintData.Name, constraintData.Name, icon: IconPath);
            }
        ...
        private static string GetDisplay(string currentValue, SkeletonData skeletonData)
        {
            ...
            foreach (PathConstraintData constraintData in GetPathConstraintData(skeletonData))
            {
                ...
                {
                    return $"<icon={IconPath}/>{currentValue}";
                }
            }
        ...
        protected override void DrawField(Rect position, SerializedProperty property, GUIContent label,
            ...
            object parent)
        {
            InfoIMGUI cache = EnsureKey(property, (SpinePathConstraintPickerAttribute)saintsAttribute, info, parent);
            if (property.propertyType != SerializedPropertyType.String)
            {
        ...
        protected override bool WillDrawBelow(SerializedProperty property, IReadOnlyList<PropertyAttribute> allAttributes,
            ...
            EnsureKey(property, (SpinePathConstraintPickerAttribute)saintsAttribute, info, parent).Error != "";
        ...
        {
            string error = EnsureKey(property, (SpinePathConstraintPickerAttribute)saintsAttribute, info, parent).Error;
