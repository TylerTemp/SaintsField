    ...
    public partial class FlagsTreeDropdownAttributeDrawer
    {
            ...
            FieldInfo info, object parent)
        {
            ...
            if (EditorGUI.DropdownButton(fieldRect,
                    new GUIContent(GetSelectedNames(metaInfo.BitValueToName, currentMask)),
                    FocusType.Keyboard))
            {
                PopupWindow.Show(fieldRect, new SaintsTreeDropdownIMGUI(
                    ...
                    (curItem, on) =>
                    {
                        ...
                        ReflectUtils.SetValue(property.propertyPath, property.serializedObject.targetObject, info,
                            parent, System.Enum.ToObject(metaInfo.EnumType, newMask));
                        if (ExpandableIMGUIScoop.IsInScoop)
                        {
                            property.serializedObject.ApplyModifiedProperties();
                        }

                        return EnumFlagsUtil.GetDropdownMetaInfo(newMask, metaInfo.AllCheckedLong,
                            metaInfo.BitValueToName).CurValues;
                    }));
            }
        }

        private static string GetSelectedNames(
            IReadOnlyDictionary<long, EnumFlagsUtil.EnumDisplayInfo> bitValueToName, long selectedInt)
        {
            string[] names = bitValueToName
                .Where(kv => EnumFlagsUtil.IsOn(selectedInt, kv.Key))
                .Select(kv => kv.Value.HasRichName ? kv.Value.RichName.Split('/').Last() : kv.Value.Name)
                .ToArray();
        ...
        protected override bool WillDrawBelow(SerializedProperty property,
            ...
            int index, FieldInfo info, object parent) => UpdateStatus(property, info).Error != "";
