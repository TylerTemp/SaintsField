...
namespace SaintsField.Editor.Drawers.MaxValueDrawer
{
    ...
    {
        private static (IReadOnlyList<string> errors, IReadOnlyList<(string message, Action fix)> checkerResults) CheckPropertyValue(SerializedProperty property, MaxValueAttribute maxValueAttribute, Action<object> onValueChangedCallback, MemberInfo info, object parent)
        {
        ...
        private static int ToInt32Floor(object obj)
        {
            // ReSharper disable once ConvertSwitchStatementToSwitchExpression
            switch (obj)
            {
        ...
        private static long ToInt64Floor(object obj)
        {
            switch (obj)
            {
                case double d:
                ...
                case decimal dec:
                    return (long)Math.Floor(dec);
                default:
                    return Convert.ToInt64(obj);
            }
        }

        private static uint ToUInt32Floor(object obj)
        {
            switch (obj)
            {
                case double d:
                    return d < 0 ? 0u : (uint)Math.Floor(d);
                case float f:
                    return f < 0 ? 0u : (uint)Math.Floor(f);
                case decimal dec:
                    return dec < 0 ? 0u : (uint)Math.Floor(dec);
                default:
                {
        ...
        private static ulong ToUInt64Floor(object obj)
        {
            switch (obj)
            {
                case double d:
                    return d < 0 ? 0ul : (ulong)Math.Floor(d);
                case float f:
                    return f < 0 ? 0ul : (ulong)Math.Floor(f);
                case decimal dec:
                    return dec < 0 ? 0ul : (ulong)Math.Floor(dec);
                default:
                {
                    long signed;
                    try
        ...
        private static (string propDirectError, NumberLimitParam propDirectNumber, Action<object> apply) ParsePropertyDirect(SerializedProperty property)
        {
            ...
            switch (property.numericType)
            {
                ...
                case SerializedPropertyNumericType.Int32:  // ..., int, use int
                    ...
                    {
                        property.intValue = ToInt32Floor(obj);
                ...
                case SerializedPropertyNumericType.UInt64: // ulong, use ulong
                    ...
                    {
                        property.ulongValue = ToUInt64Floor(obj);
                        property.serializedObject.ApplyModifiedProperties();
                    });
            ...
            switch (property.propertyType)
            {
                ...
                case SerializedPropertyType.Vector2Int:
                    return index switch
                    {
                        ...
                        {
                            property.vector2IntValue = new Vector2Int(ToInt32Floor(obj), property.vector2IntValue.y);
                            property.serializedObject.ApplyModifiedProperties();
                        }),
                        1 => (string.Empty, new NumberLimitParam(property.vector2IntValue.y), (Action<object>)(obj =>
                        {
                            property.vector2IntValue = new Vector2Int(property.vector2IntValue.x, ToInt32Floor(obj));
                            property.serializedObject.ApplyModifiedProperties();
                        })),
                ...
                case SerializedPropertyType.Vector3Int:
                    return index switch
                    {
                        ...
                        {
                            property.vector3IntValue = new Vector3Int(ToInt32Floor(obj), property.vector3IntValue.y, property.vector3IntValue.z);
                            property.serializedObject.ApplyModifiedProperties();
                        }),
                        1 => (string.Empty, new NumberLimitParam(property.vector3IntValue.y), obj =>
                        {
                            property.vector3IntValue = new  Vector3Int(property.vector3IntValue.x, ToInt32Floor(obj), property.vector3IntValue.z);
                            property.serializedObject.ApplyModifiedProperties();
                        }),
                        2 => (string.Empty, new NumberLimitParam(property.vector3IntValue.z), (Action<object>)(obj =>
                        {
                            property.vector3IntValue = new Vector3Int(property.vector3IntValue.x, property.vector3IntValue.y, ToInt32Floor(obj));
                            property.serializedObject.ApplyModifiedProperties();
                        })),
                ...
                case SerializedPropertyType.RectInt:
                    return index switch
                    {
                        0 => (string.Empty, new NumberLimitParam(property.rectIntValue.x), obj =>
                        {
                            ...
                            property.rectIntValue = new RectInt(ToInt32Floor(obj), v.y, v.width, v.height);
                            property.serializedObject.ApplyModifiedProperties();
                        }),
                        1 => (string.Empty, new NumberLimitParam(property.rectIntValue.y), obj =>
                        {
                            RectInt v = property.rectIntValue;
                            property.rectIntValue = new RectInt(v.x, ToInt32Floor(obj), v.width, v.height);
                            property.serializedObject.ApplyModifiedProperties();
                        }),
                        2 => (string.Empty, new NumberLimitParam(property.rectIntValue.width), obj =>
                        {
                            RectInt v = property.rectIntValue;
                            property.rectIntValue = new RectInt(v.x, v.y, ToInt32Floor(obj), v.height);
                            property.serializedObject.ApplyModifiedProperties();
                        }),
                        3 => (string.Empty, new NumberLimitParam(property.rectIntValue.height), (Action<object>)(obj =>
                        {
                            RectInt v = property.rectIntValue;
                            property.rectIntValue = new RectInt(v.x, v.y, v.width, ToInt32Floor(obj));
                            property.serializedObject.ApplyModifiedProperties();
                        })),
                ...
                case SerializedPropertyType.Bounds:
                    return index switch
                    {
                        ...
                        5 => (string.Empty, new NumberLimitParam(property.boundsValue.extents.z), obj =>
                        {
                            property.boundsValue = new Bounds
                            {
                                center = property.boundsValue.center,
                    ...
                    return index switch
                    {
                        0 => (string.Empty, new NumberLimitParam(property.boundsIntValue.position.x), obj =>
                        {
                            property.boundsIntValue = new BoundsInt {
                                position = new Vector3Int(ToInt32Floor(obj), property.boundsIntValue.position.y, property.boundsIntValue.position.z),
                        ...
                        {
                            property.boundsIntValue = new BoundsInt {
                                position =new Vector3Int(property.boundsIntValue.position.x, ToInt32Floor(obj), property.boundsIntValue.position.z),
                                size =property.boundsIntValue.size,
                            };
                            property.serializedObject.ApplyModifiedProperties();
                        }),
                        ...
                        {
                            property.boundsIntValue = new BoundsInt {
                                position =new Vector3Int(property.boundsIntValue.position.x, property.boundsIntValue.position.y, ToInt32Floor(obj)),
                                size =property.boundsIntValue.size,
                            };
                            property.serializedObject.ApplyModifiedProperties();
                        }),
                        3 => (string.Empty, new NumberLimitParam(property.boundsIntValue.size.x), obj =>
                        {
                            property.boundsIntValue = new BoundsInt
                            {
                                ...
                                size =new Vector3Int(ToInt32Floor(obj), property.boundsIntValue.size.y, property.boundsIntValue.size.z),
                            };
                            property.serializedObject.ApplyModifiedProperties();
                        }),
                        4 => (string.Empty, new NumberLimitParam(property.boundsIntValue.size.y), obj =>
                        {
                            ...
                            {
                                position =property.boundsIntValue.position,
                                size =new Vector3Int(property.boundsIntValue.size.x, ToInt32Floor(obj), property.boundsIntValue.size.z),
                            };
                            property.serializedObject.ApplyModifiedProperties();
                        }),
                        5 => (string.Empty, new NumberLimitParam(property.boundsIntValue.size.z), obj =>
                        {
                            property.boundsIntValue = new BoundsInt
                            {
                                position =property.boundsIntValue.position,
                                size =new Vector3Int(property.boundsIntValue.size.x, property.boundsIntValue.size.y, ToInt32Floor(obj)),
                            };
                            property.serializedObject.ApplyModifiedProperties();
                        }),
                ...
                case SourceType.Long:
                    switch (requiredLimit.SourceType)
                    {
                        case SourceType.Long:
                            ...
                            {
                                return ($"Max {requiredLimit.LongV}, get {sourceNumber.LongV}", () => apply(requiredLimit.LongV));
                            }
                            break;
                        case SourceType.Ulong:
                            if (sourceNumber.LongV >= 0 && (ulong)sourceNumber.LongV > requiredLimit.UlongV)
                            {
                                return ($"Max {requiredLimit.UlongV}, get {sourceNumber.LongV}", () => apply(requiredLimit.LongV));
                            }
                            break;
                        case SourceType.Double:
                        {
                            double flooredLimit = Math.Floor(requiredLimit.DoubleV);
                            if (sourceNumber.LongV > flooredLimit)
                            {
                                return ($"Max {flooredLimit}, get {sourceNumber.LongV}", () => apply(flooredLimit));
                            }
                            break;
                        }
                        case SourceType.Decimal:
                        {
                            decimal flooredLimit = Math.Floor(requiredLimit.DecimalV);
                            if (sourceNumber.LongV > flooredLimit)
                            {
                                return ($"Max {flooredLimit}, get  {sourceNumber.LongV}", () => apply(flooredLimit));
                            }
                ...
                case SourceType.Ulong:
                    switch (requiredLimit.SourceType)
                    {
                        case SourceType.Long:
                            ...
                            {
                                return ($"Max {requiredLimit.LongV}, get {sourceNumber.UlongV}",
                        ...
                        case SourceType.Ulong:
                            ...
                            {
                                return ($"Max {requiredLimit.UlongV}, get {sourceNumber.UlongV}",
                                    () => apply(requiredLimit.UlongV));
                            }
                            break;
                        case SourceType.Double:
                        {
                            // Source is unsigned integer; floor the limit. If limit is negative, clamp to 0.
                            double flooredLimit = requiredLimit.DoubleV < 0 ? 0 : Math.Floor(requiredLimit.DoubleV);
                            if (sourceNumber.UlongV > flooredLimit)
                            {
                                apply(flooredLimit);
                            }
                            break;
                        ...
                        case SourceType.Decimal:
                        {
                            decimal flooredLimit = requiredLimit.DecimalV < 0 ? 0m : Math.Floor(requiredLimit.DecimalV);
                            if (sourceNumber.UlongV > flooredLimit)
                            {
                                return ($"Max  {flooredLimit}, get {sourceNumber.UlongV}", () => apply(flooredLimit));
                            }
                            break;
                        }
                        case SourceType.NotSupported:
                ...
                case SourceType.Double:
                    switch (requiredLimit.SourceType)
                    {
                        case SourceType.Long:
                            ...
                            {
                                return ($"Max {requiredLimit.LongV}, get {sourceNumber.DoubleV}", ()  => apply(requiredLimit.LongV));
                            }
                            break;
                        case SourceType.Ulong:
                            if (sourceNumber.DoubleV > requiredLimit.UlongV)
                            {
                                return ($"Max {requiredLimit.UlongV}, get {sourceNumber.DoubleV}", () => apply(requiredLimit.UlongV));
                            }
                            break;
                        case SourceType.Double:
                            if (sourceNumber.DoubleV > requiredLimit.DoubleV)
                            {
                                return ($"Max {requiredLimit}, get {sourceNumber}", () => apply(requiredLimit.DoubleV));
                            }
                            break;
                        case SourceType.Decimal:
                            if ((decimal)sourceNumber.DoubleV > requiredLimit.DecimalV)
                            {
                                return ($"Max {requiredLimit}, get {sourceNumber}", () => apply(requiredLimit.DecimalV));
                            }
                            break;
                        case SourceType.NotSupported:
                        case SourceType.NoLimit:
                ...
                case SourceType.Decimal:
                    switch (requiredLimit.SourceType)
                    {
                        case SourceType.Long:
                            ...
                            {
                                return ($"Max {requiredLimit}, get {sourceNumber}", () => apply(requiredLimit.LongV));
                            }
                            break;
                        case SourceType.Ulong:
                            if (sourceNumber.DecimalV > requiredLimit.UlongV)
                            {
                                return ($"Max {requiredLimit}, get {sourceNumber}", () => apply(requiredLimit.UlongV));
                            }
                            break;
                        case SourceType.Double:
            ...
            SerializedProperty property, MemberInfo memberInfo, object parent)
        {
            ...
            (IReadOnlyList<string> errors, IReadOnlyList<(string message, Action fix)> checkerResults) = CheckPropertyValue(property, maxValueAttribute, _ => { }, memberInfo, parent);
