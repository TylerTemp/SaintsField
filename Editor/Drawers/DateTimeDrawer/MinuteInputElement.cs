...
namespace SaintsField.Editor.Drawers.DateTimeDrawer
{
    ...
    public partial class MinuteInputElement: BindableElement, INotifyValueChanged<long>
    {
        public readonly IntegerField IntegerField;
        private long _cachedValue;

        // ReSharper disable once MemberCanBePrivate.Global
        public MinuteInputElement()
        {
            ...
            UIToolkitUtils.MakePlaceholderRight(IntegerField.Q<VisualElement>("unity-text-input"), DateTimeUtils.GetMinuteLabel());
            IntegerField.RegisterValueChangedCallback(evt =>
            {
                DateTime dt = new DateTime(_cachedValue);
                int newValue = Mathf.Clamp(evt.newValue, 0, 59);
                value = new DateTime(dt.Year, dt.Month, dt.Day, dt.Hour, newValue, dt.Second, dt.Millisecond, dt.Kind).Ticks;
            });
            Add(IntegerField);
        }
        ...
        public void SetValueWithoutNotify(long newValue)
        {
            ...
            IntegerField.SetValueWithoutNotify(minute);
        }
